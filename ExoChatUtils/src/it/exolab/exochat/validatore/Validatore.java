...
public class Validatore {
	...
	public void validatoreUtente(Utente utente) throws BusinessException {
		...
		if(null == utente.getEmail() && utente.getEmail().length() <= 8 && utente.getEmail().length() >= 30 && !(utente.getEmail().matches(Costanti.REGEX_EMAIL))) {
		...
		if(null == utente.getPassword() && utente.getPassword().length() <= 8 && utente.getPassword().length() >= 20 && !(utente.getPassword().matches(Costanti.REGEX_PASSWORD))) {
